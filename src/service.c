...
void update_setup_calendar() {
    ...
    *address = 0x0000;
    ...
    address = _add_large_char(address, 0, 0x720C, *BIOS_YEAR);
    address = _add_large_char(address, 0, 0x714C, *BIOS_MONTH);
    address = _add_large_char(address, 0, 0x71AC, *BIOS_DAY);
    address = _add_large_char(address, 0, 0x726C, *BIOS_HOUR);
    address = _add_large_char(address, 0, 0x72CC, *BIOS_MINUTE);
    address = _add_large_char(address, 0, 0x732C, *BIOS_SECOND);
    /*
    address = _add_large_char(address, 0, 0x714D, *BIOS_HOUR);
    address = _add_large_char(address, 0, 0x71AD, *BIOS_MINUTE);
    address = _add_large_char(address, 0, 0x720D, *BIOS_SECOND);
    */
/*
    *address = 0x0003;
    address++;
    ...
    *address = 0x0108;
    address++;    
    for (uint8_t i = 0; i < 3; i++) {
        *(volatile uint8_t *)address = weekdays[*BIOS_WEEKDAY][i];
        address = (volatile uint16_t *)((volatile uint8_t *)address + 1);
    }
    *(volatile uint8_t *)address = 0xFF;
    address = (volatile uint16_t *)((volatile uint8_t *)address + 1);
*/
    ...
    // Change date section
    address = _add_large_char(address, (*SERVICE_CALENDAR_CURSOR & 1  ? 1 : 0), 0x7153, *NEW_BIOS_MONTH);
    address = _add_large_char(address, (*SERVICE_CALENDAR_CURSOR & 2  ? 1 : 0), 0x71B3, *NEW_BIOS_DAY);
    address = _add_large_char(address, (*SERVICE_CALENDAR_CURSOR & 4  ? 1 : 0), 0x7213, *NEW_BIOS_YEAR);
    address = _add_large_char(address, (*SERVICE_CALENDAR_CURSOR & 8  ? 1 : 0), 0x7273, *NEW_BIOS_HOUR);
    address = _add_large_char(address, (*SERVICE_CALENDAR_CURSOR & 16 ? 1 : 0), 0x72D3, *NEW_BIOS_MINUTE);
    address = _add_large_char(address, (*SERVICE_CALENDAR_CURSOR & 32 ? 1 : 0), 0x7333, *NEW_BIOS_SECOND);
